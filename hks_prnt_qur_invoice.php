...
if ($result_all->num_rows > 0) {
 ...
 $newDateString = date_format(date_create_from_format('Y-m-d', $row["SentDate"]), 'd/m/Y');
  ...
  echo '
           ...
           <tr > <td style="font-size:22px; text-align: Right; font-weight: bold;" colspan="2" > '.id_cour_name($row["transid"]).',  '.id_cour_description($row["transid"]).'</td></tr>
           <tr style="font-size:14px; text-align: center;  font-weight: bold;"><td> Agent: '.$row["transid"].'</td><td> Issue: '.$print_issue.'</td></tr>
           <tr>
...
থানাঃ'.ps_en_bn(id_ps($row["transid"])).', <br>
 ...
 '.id_phone($row["transid"]).' 
           ...
           <img style="width:100%;height:70%;" id="s'.$row["transid"].'" >
           ...
           <tr  ><td style="font-size:24px;">HKS-'.$print_issue.'</td><td style="font-size:24px;">'.doubleval($row["agcpy"]).'</td><td  style="font-size:24px;" >'.doubleval(id_news_rate($row["transid"])).'</td><td style="font-size:24px;" >'.intval(doubleval($row["agcpy"])*doubleval(id_news_rate($row["transid"]))).'</td></tr> 
           ...
           <div style="font-family:  SutonnyMJ;text-align: Center; font-weight: bold; margin-top: 100px;">
           
           <h4>কন্ডিশন: (<span style=" font-family:  SutonnyMJ">'.$new_num.'</span>/-)  <span id="w'.$row["transid"].'" ></span>
