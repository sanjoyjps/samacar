...
(select sum(news_price)    from  tblIssue ti where ti.vername=pd.issue  ) as sale,
(select sum(Speciment)    from  tblIssue ti where ti.vername=pd.issue  ) as Speciment,
...
<input type="hidden" name="issue" value="'.$row["issue"].'">
<td> '.doubleval($row["np"])/doubleval($row["nr"]).'</td>
<td>'.doubleval($row["sale"]).'</td>
<td>'.doubleval($row["Speciment"]).' </td>

<td>'.doubleval($row["wastage"]).' </td>
<td>'.doubleval(doubleval($row["quantity"])-doubleval($row["sale"])-doubleval($row["Speciment"])
