...
$tran_date=intval(date("Ymd"));
  ...
  $sb_ps=$_POST["sb_ps"];
  $sb_po=$_POST["sb_po"];
  $sb_poffice_bn=$_POST["sb_poffice_bn"];
...
?>
<tr><td>Name: <?php echo $sb_name;?></td></tr>

 

<tr><td>Address: <?php echo $sb_addr.$sb_po.$sb_ps.$sb_dist;?></td></tr>
 


<tr><td>Mobile: <?php echo $sb_phone1.$sb_phone2;?></td></tr>
</table>
</div>
<br>
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO tblMem (ID,ID_EN,Name,vill,po,ps,dist,cont,phone,mob,email,user,balance,comment,ag_date,status)
    VALUES (:ID,:ID_EN,:Name,:vill,:po,:ps,:dist,:cont,:phone,:mob,:email,:user,:balance,:comment,:ag_date,:status)");
    ...
    $stmt->bindParam(':phone', $sb_phone1);
    $stmt->bindParam(':mob', $sb_phone2);
