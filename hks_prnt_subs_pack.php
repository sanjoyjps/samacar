...
if(intval(id_balance($row["transid"]))>0){


           $news_warning='আপনি আগামী '.$dt.' পর্যন্ত '.id_balance($row["transid"]).' টি পত্রিকা পাবেন।';
        ...
        if(intval(id_balance($row["transid"]))<1){
...
<div class="page">
        ...
        <tr><td>RP No.</td> <td><b> '.substr($row["VPNo"],2).'</b></td></tr>
        </td>
        </table>
...
ডাকঃ'.id_po_bn($row["transid"]).",থানাঃ".ps_en_bn(id_ps($row["transid"])).', <br>
 ...
 '.id_phone($row["transid"]).'
        ...
        গ্রাহক পদের মেয়াদ নবায়নের জন্য যোগাযোগ করুন- ০১৭১৫ ৭৫৮৯৪৮। </td>
        <td>
