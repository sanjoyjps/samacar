...
$sql = "SELECT transid,vername, SentDate,agcpy ,count(transid) FROM tblIssue where (transid >99 AND transid <10000 AND vername =".$print_issue." ) GROUP BY agcpy";
...
echo '<tr><td>মোট</td><th>'.$sum_post1.'</th><th>'.$sum_post2.'</th><th>'.$sum_post3.'</th><td></td><th></th><td></td><td></td></tr>';
...
$sql = "SELECT transid,vername, SentDate,agcpy ,count(transid) FROM tblIssue where( transid <100 AND vername =".$print_issue.") GROUP BY agcpy";
...
echo '<tr><td>মোট</td><th>'.$sum_cour1.'</th><th> '.$sum_cour2.'</th><th>'.$sum_cour3.'</th><td></td><th></th><td></td><td></td></tr></table>In Total :';
  $s= $sum_sub1+$sum_post1+ $sum_cour1; 
 
 $d= $sum_sub2+$sum_sub3+$sum_post2+$sum_post3+ $sum_cour2+$sum_cour3; 
